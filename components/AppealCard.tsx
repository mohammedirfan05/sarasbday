...
export default function AppealCard({
  ...
  const handleCardDoubleTap = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (now - lastTapRef.current < 320) {
      ...
      sound.playPop();
      triggerHaptic(25);
      fireMiniBurst(x / rect.width, y / rect.height);
      setTimeout(() => {
  ...
  const canSwipeCard = isFlipped || isDismissed || flaw.interactionType !== "scratch";

  const isFlipType = flaw.interactionType === "flip";

  return (
    <div className="relative w-full max-w-md mx-auto flex flex-col justify-between py-2 px-1 [perspective:1200px]">
      {/* Top Docket Bar */}
      ...
      {/* Main Physical Scrapbook Card with 3D Flip & Swipe */}
      <motion.div
        key={flaw.id}
        drag={canSwipeCard ? "x" : false}
        ...
        animate={{
          ...
          opacity: 1,
          y: 0,
          rotateY: isFlipType && isFlipped ? 180 : 0,
        }}
        ...
        className={`relative w-full transition-transform touch-pan-y ${
      ...
      >
        ...
        <div
          ...
          style={{
            backfaceVisibility: isFlipType ? "hidden" : "visible",
            WebkitBackfaceVisibility: isFlipType ? "hidden" : "visible",
          }}
          className={`relative w-full rounded-2xl scrapbook-paper text-[#1A1918] p-5 sm:p-6 overflow-hidden shadow-2xl ${
            isFlipType && isFlipped ? "pointer-events-none" : ""
          }`}
        >
          {/* Double Tap Floating Heart Bursts */}
          <AnimatePresence>
            {doubleTapHearts.map((h) => (
              <motion.div
                key={h.id}
                initial={{ scale: 0, opacity: 1, y: 0 }}
                animate={{ scale: [0, 1.4, 1.2], opacity: [1, 1, 0], y: -55 }}
                exit={{ opacity: 0 }}
                transition={{ duration: 0.75, ease: "easeOut" }}
                style={{ left: h.x - 24, top: h.y - 24 }}
                className="absolute pointer-events-none z-50 text-rose-500"
              >
                <Heart className="w-12 h-12 fill-rose-500 drop-shadow-lg" />
              </motion.div>
            ))}
          </AnimatePresence>
          ...
          {/* Interaction Trigger Area for unflipped state */}
          {(!isFlipped || isFlipType) && (
            <div className="my-5 p-4 rounded-xl bg-white/70 border border-[#E2DDD5] text-center">
              {flaw.interactionType === "stamp" && !isFlipped && (
                <button
              ...
              {flaw.interactionType === "flip" && (
                ...
                >
                  <RotateCw className="w-4 h-4 text-amber-300 animate-spin" style={{ animationDuration: '4s' }} />
                  <span>Flip Card to View Defense</span>
                </button>
              )}

              {flaw.interactionType === "hold" && !isFlipped && (
                <button
              ...
              {flaw.interactionType === "scratch" && !isFlipped && (
          ...
          {/* Non-flip inline revealed defense */}
          {!isFlipType && isFlipped && (
            <motion.div
        ...
        {isFlipType && (
          <div
            ...
            className={`absolute inset-0 w-full h-full rounded-2xl scrapbook-paper text-[#1A1918] p-5 sm:p-6 overflow-y-auto shadow-2xl flex flex-col justify-between ${
          ...
          >
            ...
            <div>
              ...
              <div className="flex items-center justify-between mb-3 pt-1">
                ...
                <button
                  onClick={handleFlipToggle}
                  className="flex items-center gap-1 font-mono text-[10px] text-emerald-800 font-bold uppercase tracking-wider bg-emerald-100/90 hover:bg-emerald-200/90 px-2.5 py-0.5 rounded-full border border-emerald-300 transition-colors cursor-pointer"
                >
                  <CheckCircle2 className="w-3 h-3 text-emerald-700" />
                  <span>Dismissed &bull; Flip</span>
                </button>
              </div>
            ...
            {/* Footer Fun Fact & Re-flip option */}
            <div className="pt-2">
              {flaw.funFact && (
                <span className="font-mono text-[10px] text-[#5A5752] block text-center mb-2">
                  &bull; {flaw.funFact}
                </span>
              )}
              <button
                onClick={handleFlipToggle}
                className="w-full py-1.5 text-center text-[10px] font-mono text-[#5A5752] hover:text-[#1A1918] transition-colors flex items-center justify-center gap-1 cursor-pointer"
              >
                <RotateCw className="w-3 h-3" />
                <span>Tap to flip back to charge</span>
              </button>
            </div>
          </div>
        )}
